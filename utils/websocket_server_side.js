...
module.exports = function (cp, fcw, logger) {
	...
	ws_server.process_msg = function (ws, data) {
		...
		// create create_a_product
		else if (data.type === 'product_regist') {
			logger.info('[ws] create product req');
			options.args = {
				...
				mainingredientname: data.mainingredientname,
				forcoin: data.forcoin
			};

			marbles_lib.create_a_product(options, function (err, resp) {
				if (err != null) send_err(err, data);
				else options.ws.send(JSON.stringify({ msg: 'tx_step', state: 'finished' }));
			});
		}

		// create create_a_material
		else if (data.type === 'material_regist') {
			logger.info('[ws] create material req');
			options.args = {
				groupid: data.groupid,
				groupname: data.groupname,
				registeruserid : data.registeruserid,
			...
			marbles_lib.create_a_material(options, function (err, resp) {
				if (err != null) send_err(err, data);
				else options.ws.send(JSON.stringify({ msg: 'tx_step', state: 'finished' }));
			});
		}	

		// create create_a_contract
		else if (data.type === 'contract_regist') {
			/*
			...
			*/
			logger.info('[ws] create contract req');
			options.args = {
				supplygroupid: data.supplygroupid,
				supplygroupname : data.supplygroupname,
				developgroupid : data.developgroupid,
				developgroupname: data.developgroupname,
				registersupplyuserid: data.registersupplyuserid,
				registerdevelopuserid: data.registerdevelopuserid,
				registersupplyusername: data.registersupplyusername,
				registerdevelopusername : data.registerdevelopusername,
				issupplygroupconfirm : data.issupplygroupconfirm,
				isdevelopgroupconfirm : data.isdevelopgroupconfirm,
			...
			marbles_lib.create_a_contract(options, function (err, resp) {
				if (err != null) send_err(err, data);
				else options.ws.send(JSON.stringify({ msg: 'tx_step', state: 'finished' }));
			});
		}		
		...
		// create read_product
		...
		else if (data.type === 'read_product_req') {
			logger.info('[ws] read_product req');
			read_product(ws);
			//marbles_lib.read_product(options, function (err, resp) {
			//	if (err != null) send_err(err, data);
			//	else options.ws.send(JSON.stringify({ msg: 'read_product', state: 'finished' }));
			//});
		}

		else if (data.type === 'login_req') {
			logger.info('[ws] login req');
			
			if(data.id === 'taeho' && data.pw === '1234'){
				sendMsg({ 
					msg: 'login_result', 
					result: 'success', 
					user_id: 'taeho', 
			...
			else if(data.id === 'sarang' && data.pw === '1234'){
				sendMsg({ 
					msg: 'login_result', 
					result: 'success', 
					user_id: 'sarang', 
	...
	function read_product(ws_client) {
		const channel = cp.getChannelId();
		const first_peer = cp.getFirstPeerName(channel);
		var options = {
			peer_urls: [cp.getPeersUrl(first_peer)],
		...
		marbles_lib.read_product(options, function (err, resp) {
			if (err != null) {
				...
				if (data && data.product) {
					console.log('');
					logger.debug('[checking] number of product:', data.product.length);
					//logger.debug('[checking] number of marbles:', data.marbles.length);
				}

				logger.debug('[checking] product list test');
				//known_everything = data;

				//서버사이드 측으로 브로드캐스트를 한다.
				wss.broadcast({ msg: 'read_product', e: err, everything: data });	//sent to all clients

				//ws_client.send(JSON.stringify(obj));
				//ws_client.send(JSON.stringify({ msg: 'read_product', e: err, everything: data }));
				/*
				data.owners = organize_usernames(data.owners);
				data.marbles = organize_marbles(data.marbles);
				var knownAsString = JSON.stringify(known_everything);			//stringify for easy comparison (order should stay the same)
