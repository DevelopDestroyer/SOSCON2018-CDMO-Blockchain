...
module.exports = function (enrollObj, g_options, fcw, logger) {
	...
	//create a product
	marbles_chaincode.create_a_product = function (options, cb) {
		console.log('[marble_cc] Creating a product...');
		...
		var opts = {
			...
			endorsed_hook: options.endorsed_hook,
			ordered_hook: options.ordered_hook,
			cc_function: 'init_product',
			cc_args: [
				options.args.groupid,
				options.args.groupname,
				options.args.registeruserid,
				options.args.registerusername,
				'p' + leftPad(Date.now() + randStr(5), 19),
				options.args.registerdate,
				options.args.productname,
				options.args.productweight,
				options.args.productvolume,
				options.args.mainingredientname,
				options.args.forcoin
			],
		};
		fcw.invoke_chaincode(enrollObj, opts, function (err, resp) {
			if (cb) {
				...
				resp.id = opts.cc_args[0];			//pass marble id back
				cb(err, resp);
				console.log('[marble_cc] response : ' + resp.id + " and.. " + opts);
				logger.info('[marble_cc] result : ' + opts);
			}
		});
	};
	...
	//create a material
	marbles_chaincode.create_a_material = function (options, cb) {
		console.log('[marble_cc] Creating a material...');
		logger.info('Creating a material...');
		...
		var opts = {
			...
			endorsed_hook: options.endorsed_hook,
			ordered_hook: options.ordered_hook,
			cc_function: 'init_material',
			cc_args: [
				options.args.groupid,
				options.args.groupname,
				options.args.registeruserid,
				options.args.registerusername,
				't' + leftPad(Date.now() + randStr(5), 19),
				options.args.materialname,
				options.args.registerdate,
				options.args.materialweight,
				options.args.forcoin
			],
		};
		fcw.invoke_chaincode(enrollObj, opts, function (err, resp) {
	...
	//create a contract
	marbles_chaincode.create_a_contract = function (options, cb) {
		console.log('[marble_cc] Creating a contract...');
		logger.info('Creating a contract...');
		...
		var opts = {
			...
			endorsed_hook: options.endorsed_hook,
			ordered_hook: options.ordered_hook,
			cc_function: 'init_contract',
			cc_args: [
				'c' + leftPad(Date.now() + randStr(5), 19),
				options.args.supplygroupid,
				options.args.supplygroupname,
				options.args.developgroupid,
				options.args.developgroupname,
				options.args.registersupplyuserid,
				options.args.registerdevelopuserid,
				options.args.registersupplyusername,
				options.args.registerdevelopusername,
				options.args.issupplygroupconfirm,
				options.args.isdevelopgroupconfirm,
				options.args.materialid,
				options.args.materialname,
				options.args.materialweight,
				options.args.forcoin
	...
	//build full name
	marbles_chaincode.read_product = function (options, cb) {
		console.log('read_product');
		logger.info('Fetching read product...');
